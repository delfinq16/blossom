...
if ( ! class_exists( 'YITH_WooCommerce_Pdf_Invoice' ) ) {
	...
	class YITH_WooCommerce_Pdf_Invoice {
		...
		public function add_back_end_shipping_list_buttons( $order ) {
			if ( 'yes' == get_option( 'ywpi_enable_shipping_list' ) ) {
				...
				if ( $shipping_document->exists ) {
					...
					$text  = esc_html__( "Show shipping list document", 'yith-woocommerce-pdf-invoice' );
					$class = "ywpi_view_shipping_list";

				} else {
					$url   = ywpi_document_nonce_url( YITH_YWPI_CREATE_SHIPPING_LIST_ARG_NAME, $shipping_document );
					$text  = esc_html__( "Create shipping list document", 'yith-woocommerce-pdf-invoice' );
		...
		public function show_invoice_custom_column_data( $column ) {
			...
			echo '<small class="meta">' . sprintf( esc_html__( "Invoice No. %s of %s", 'yith-woocommerce-pdf-invoice' ), $invoice->get_formatted_invoice_number(), $invoice->get_formatted_date() ) . '</small>';
		}
		...
		function add_invoice_metabox() {
			...
			add_meta_box( 'yith-pdf-invoice-box', esc_html__( 'YITH PDF Invoice', 'yith-woocommerce-pdf-invoice' ), array(
				$this,
