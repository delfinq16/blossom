	...
	*/
	class WKWCPOS_Manage_Invoice_Template {
		...
		public function wkwcpos_get_invoice_editor_template() {
			...
			wp_localize_script('wkwcpos-invoice-script', 'wkwcposInvoiceObj',
			array(
			...
			<div class="wrap">
				<form method="get">
					...
					<input type="text" name="pos_user" value="<?php echo esc_attr( ! empty( $_GET[ 'pos_user' ] ) ? $_GET[ 'pos_user' ] : '' ); ?>" placeholder="<?php esc_attr_e( 'Enter POS User ID', 'wc_pos' ); ?>" />
					<input type="text" name="pos_order" value="<?php echo esc_attr( ! empty( $_GET[ 'pos_order' ] ) ? $_GET[ 'pos_order' ] : '' ); ?>" placeholder="<?php esc_attr_e( 'Enter User\'s Outlet\'s Order ID', 'wc_pos' ); ?>" />
