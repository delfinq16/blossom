		...
		private function table_data() {
			...
			foreach ( $pos_users as $u_key => $u_value ) {
				...
				if( empty( $my_role ) || ( !empty( $my_role ) && (in_array( 'pos_user', $my_role ) || $custom_rule )) ) {
					...
					if( !empty( $my_role ) && (in_array( 'pos_user', $my_role ) || $custom_rule) ){
...
<div class="wrap">
	...
	<h1><?php echo __( 'POS User List','wc_pos'); ?> <a href="<?php echo admin_url() . 'admin.php?page=pos-system&action=add'; ?>" class="page-title-action"><?php _e( 'Add New', 'wc_pos' ); ?></a></h1>
	...
	<form method="POST">
		...
		$wc_pos_list->search_box( __('Search Users', 'wc_pos'), 'search-id' );
