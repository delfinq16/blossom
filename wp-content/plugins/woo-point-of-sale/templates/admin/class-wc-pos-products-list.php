...
if (!class_exists('WC_Pos_Products_List')) {
    class WC_Pos_Products_List extends WP_List_Table
    {
                        ...
                        if ($p_value->is_type('variable')) {
                            ...
                            if ($p_value->get_available_variations()) {
                                foreach ($p_value->get_available_variations() as $key => $value) {
                                    ...
                                    ?>
                                    <p>
                                        <strong style="width:300px;display:inline-block;"><?php echo $variation_data->get_formatted_name(); ?></strong><span style="width:200px;display:inline-block;"><?php
                                        $barcode_image = '';
                                        if (!empty($barcode_id)) {
                                        ...
                                        $print_barcode = '<a href="'.admin_url('admin.php?page=pos-products').'" class="print-barcode" data-title="'.$title.'" data-image-id="'.$barcode_id.'" data-image-sku="'.$barcode_sku.'"  alt="barcode" '.$custom_data_value.' style="display:inline-block; position:absolute;right: 25px;"/>Print Barcode </a>';
        ...
        {
            return apply_filters('wkwcpos_add_bulk_action_in_product_page', array(
        ...
        public function column_product_name($item)
        {
            ...
            $actions = apply_filters('wkwcpos_add_coloum_action', array(
                'print_barcode' => sprintf('<a href="'.admin_url('admin.php?page=pos-products').'" class="print-barcode" data-image-id="%s" data-image-sku="%s" data-title="%s">%s</a>', $barcode_id, $barcode_sku, get_the_title($item['id']), esc_html__('Print Barcode', 'wc_pos')),
        ...
        public function extra_tablenav($which)
        {
            ...
            if ('top' === $which) {
				...
				<div class="alignleft actions bulkactions">
                    ...
                    <input type="submit" value="<?php esc_attr_e('Filter', 'wc_pos'); ?>" class="button" />
...
<div class="wrap">
    ...
    <form method="GET">
        ...
        $pos_products->search_box( esc_html__( 'Search', 'wc_pos' ), 'search-id');
