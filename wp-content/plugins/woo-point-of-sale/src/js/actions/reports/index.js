...
import { POSGetRequest } from './../../hash';
/**
 ...
 */ 
import { NAMESPACE, MAX_PER_PAGE, QUERY_DEFAULTS } from './../../api/constants';
/**
 * WooCommerce dependencies
 */
import { appendTimestamp, getCurrentDates, getIntervalForQuery } from '@woocommerce/date';
import { flattenFilters, getActiveFiltersFromQuery, getUrlKey } from '@wkwcpos/navigation';
import { formatCurrency } from '@woocommerce/currency';
  
 export const getReportChartData = async (options) => {
  ...
  if (user != "") { 
	...
	options.forEach((option, i) => {
		p[i] = new Promise((resolve, reject) => {
			AjAxGetRevenueReport(option).then((response) => { 
				
				if (response && response.data ) {
					if (option.dataType == 'primary') {
						revenueObj.primaryData.data = response.data;
					} else if (option.dataType == 'secondary') {
						revenueObj.secondaryData.data = response.data;
					}
				}
				resolve(revenueObj);
			});
...
const AjAxGetRevenueReport = async (options) => {

    const { endpoint, dataType} = options; 
	...
	requestQuery['manager_id'] = apif_script.logged_in.user_id;
	requestQuery['outlet_id'] = apif_script.logged_in.outlet_id;
	
    fetchArgs.path = addQueryArgs(`${ NAMESPACE }/reports/${ endpoint }/stats`, requestQuery);
