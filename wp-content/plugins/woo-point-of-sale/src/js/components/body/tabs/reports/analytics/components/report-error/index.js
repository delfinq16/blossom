...
import { getAdminLink } from '@wkwcpos/navigation';
...
class ReportError extends Component {
	render() {
		...
		if ( isError ) {
			title = __( 'There was an error getting your stats. Please try again.', 'wc_pos' );
			actionLabel = __( 'Reload', 'wc_pos' );
			actionCallback = () => {
				// @todo Add tracking for how often an error is displayed, and the reload action is clicked.
				window.location.reload();
			};
		} else if ( isEmpty ) {
			title = __( 'No results could be found for this date range.', 'wc_pos' );
			actionLabel = __( 'View Orders', 'wc_pos' );
