...
if (!class_exists('WC_Pos_Orders_Helper')) {
    ...
    class WC_Pos_Orders_Helper implements Util\Order_Interface
    {
        ...
        public function __construct()
        {
            ...
            $this->order_status_list = implode(',', array_map(function ($val) {
        ...
        public function pos_get_all_order_by_search_count($search_query = '', $pay_method = '')
        {
            ...
            if (!empty($search_query) && !empty($pay_method)) {
                $result = $this->wpdb->get_row($this->wpdb->prepare("SELECT count(*) FROM $this->table_name_post as posts , $this->table_name as postmeta,$this->table_name as postmeta2  where posts.ID = postmeta.post_id AND
				posts.ID = postmeta2.post_id AND posts.post_status IN (".$this->order_status_list.") AND posts.post_type = 'shop_order' AND  postmeta.meta_key = '_wk_wc_pos_outlet' AND postmeta2.meta_key = '_payment_method' AND postmeta2.meta_value = %s AND posts.ID=%d", $search_query, $pay_method), ARRAY_A);
            } elseif (empty($search_query) && !empty($pay_method)) {
                $result = $this->wpdb->get_row($this->wpdb->prepare("SELECT count(*) FROM $this->table_name_post as posts , $this->table_name as postmeta,$this->table_name as postmeta2  where posts.ID = postmeta.post_id AND
				posts.ID = postmeta2.post_id AND posts.post_status IN (".$this->order_status_list.") AND posts.post_type = 'shop_order' AND  postmeta.meta_key = '_wk_wc_pos_outlet' AND postmeta2.meta_key = '_payment_method' AND postmeta2.meta_value = %s", $pay_method), ARRAY_A);
            } elseif (!empty($search_query) && empty($pay_method)) {
