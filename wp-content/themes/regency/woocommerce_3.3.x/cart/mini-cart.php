...
<?php if ( ! WC()->cart->is_empty() ) : ?>
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<li class="woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
                        ...
                        <?php else : ?>
                            <a href="<?php echo esc_url( $product_permalink ); ?>" class="mini-cart-thumb">
    ...
    <p class="woocommerce-mini-cart__total total"><span><?php _e( 'Subtotal', 'woocommerce' ); ?></span> <?php echo WC()->cart->get_cart_subtotal(); ?></p>

    <?php do_action( 'woocommerce_widget_shopping_cart_before_buttons' ); ?>

    <p class="woocommerce-mini-cart__buttons buttons">
        <a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="button wc-forward btn-ghost"><?php _e( 'View Cart', 'yit' ); ?></a>
        <a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="button checkout wc-forward btn-flat"><?php _e( 'Checkout', 'yit' ); ?></a>
    </p>
